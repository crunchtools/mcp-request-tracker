...
server.py (@mcp.tool)
    | validates args
    v
tools/tickets.py (async function)
    | builds params/content
    v
client.py (httpx POST)
    | form-data auth
    v
RT REST 1.0 API
```
...
- [ ] Add async functions to `tools/tickets.py`
...
- [ ] `uv run mypy src`
- [ ] `RT_URL=https://rt.example.com RT_USER=test RT_PASS=test uv run pytest -v`
- [ ] `gourmand --full .`
...
- [ ] `test_*` — verify response shape with `_mock_rt_response()`
